...
from .serializers import (
    IssueSerializer, IssueDetailSerializer, IssueStatusSerializer,
    CommentSerializer, AttachmentSerializer, AssignIssueSerializer,
...
class IssueViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if status_param:
            ...
            issue_ids = IssueStatus.objects.filter(status=status_param).values_list('issue_id', flat=True)
    ...
    @action(detail=True, methods=['post'], permission_classes=[permissions.IsAuthenticated, IsFacultyUser])
    def escalate(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            return Response({"message": "Issue escalated successfully"}, status=status.HTTP_200_OK)
